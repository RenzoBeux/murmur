...
**Post-plan follow-up shipped (2026-07-13) — branch `feat/remediation-soft-delete`, stacked on wave-3c.** A large batch of the deferred/backlog items, each committed separately and headlessly verified (**285 lib tests green** + MCP smoke test + `tsc --noEmit` clean + `bun test`). Two features (soft-delete, FTS5) also passed adversarial multi-agent review, which caught + fixed a HIGH perf cliff and a MEDIUM search-correctness bug.

Shipped commits:
- **2.6 `soft-delete-undo`** (`4761e88`) — reversible delete: `meetings.deleted_at` migration, soft `delete_meeting`, `restore_meeting`/`purge_meeting`/`purge_trash_older_than(30)` swept at all 3 DB-init entry points; every listing/search (app + MCP, MCP gracefully via a column-probe) filters trash; sidebar Undo toast.
- **tsc-fix** (`2236d54`) — pre-existing Wave 6.2 error (`SidebarItem.createdAt` missing) surfaced while verifying; branch is now genuinely tsc-clean. *Prior waves' "tsc 0 errors" was masked by `| tail` swallowing tsc's exit — always capture to a file + `grep -c "error TS"`.*
- **6.8–6.10 FTS5 unified search** (`6418d1c` + review fixes `7bffd8f`) — content-owning FTS5 `search_index` over transcripts/chunks/summaries/notes/chat, diacritic-insensitive, rowid-encoded for O(log n) trigger deletes, grouped-per-meeting in SQL; app + MCP search rewritten (MCP graceful LIKE fallback).
- **6.7 meeting-tags** (`54f17a1`) — backend + MCP `tag` scoping (frontend chips are a follow-up).
- **5C.8/5C.11 MCP smoke test + CI** (`8ce9090`) — drives the real `murmur --mcp` binary over stdio; new CI job; gate-policy docs (clippy backlog ~155, eslint stay informational).
- **3C.1/3C.3 live audio quality** (`0ad1dd8`, MANUAL VERIFY) — stateful rubato sinc VAD resampler; EBU R128 normalization of the system-VAD copy.
- **5A.6 transcripts-jsonl** (`2b2e032`, MANUAL VERIFY) — durable append-only `transcripts.jsonl` (kills the O(n²) rewrite); recovery folds it by `sequence_id`, prefers it over `transcripts.json`.
- **6.5 DB restore** (`ae17e14`, MANUAL VERIFY) — `db_restore_backup` (checkpoint→close→move-aside→swap→restart), reversible; frontend button is a follow-up.

**Still deferred (deliberately NOT blind-merged):** `keychain` 4.7/4.8 (silently loses API keys if the unverifiable keychain round-trip is wrong; P2), `bound-audio-channels` 5A.7 + `audio-ownership-thread` 5A.8 (whole-audio-path structural refactors — a blind change most risks breaking core recording; need the dedicated pass + mic-test loop the plan always intended), `live-virtualization` 5B.3 (pure visual; blind change could break live-transcript scroll). Frontend UIs for tags/restore/title-prompt need a browser-preview QA pass.
