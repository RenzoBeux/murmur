...
export function OnboardingContainer({
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 top-[var(--titlebar-height)] bg-background flex items-center justify-center z-50 overflow-hidden">
      <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-72 bg-[radial-gradient(ellipse_at_top,hsl(var(--brand)/0.08),transparent_60%)]" />
            ...
            {showNavigation && (
              <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between pointer-events-none">
                <button
                  ...
                  className={cn(
                    'pointer-events-auto w-8 h-8 rounded-full bg-elevated border border-border text-muted-foreground shadow-sm flex items-center justify-center transition-all duration-200',
                    canGoPrevious && step !== 1
                      ? 'hover:bg-accent hover:shadow-md hover:scale-110'
